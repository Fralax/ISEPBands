...
$verificationMembreBanniById = $membre->verificationMembreBanni($_GET['id']);

if ($verificationSessionExiste == true) {
  if ($verificationSessionValide == true) {
    if ($verificationMembreValide == true) {
      if ($verificationMembreBanniById == true) {
        $i = 4; //Si le membre est connecté, sur sa page de profil, et qu'il est banni
      } else{
        $i = 0; // Si le membre est connecté, sur sa page de profil, et qu'il est validé
      }
    } else{
      if ($verificationMembreBanniById == true) {
        $i = 4; //Si le membre est connecté, sur sa page de profil, qu'il n'est pas validé et qu'il est banni
      } else{
    ...
    if ($verificationMembreBanniById == true) {
      $i = 5; // Si le membre est connecté sur la page de profil d'un autre membre et que ce membre est banni
    } else{
   ...
   if ($verificationMembreBanniById == true) {
     $i = 5; //Si l'utilisateur n'est pas connecté et qu'il visionne la page d'un membre banni
   } else{
...
<div class="container">
  ...
  <?php if ($i != 4 && $i != 5): ?>
            ...
            <div class="detailsInstruments">
              ...
              <?php if ($i == 0 || $i == 2): ?>
                <form action="" method="post">
                  ...
                  <button type="submit" class="petitsBoutonsFormulaires" name="boutonsupprimerInstrumentPratique">Supprimer</button>
                </form>
              <?php endif; ?>
    ...
    <div class="row row-centered informations">
      ...
      <?php if ($i == 1): ?>
        ...
        <div>
          Vous devez être connecté et qu'un membre du bureau ait validé votre compte pour accéder aux informations personnelles de <?php echo $infos['u_prenom'] ?>.
        </div>
      ...
      <?php if ($i == 6): ?>
        ...
        <div>
          Vous avez été banni. Vous n'avez plus accès aux informations personnelles de <?php echo $infos['u_prenom']." ".$infos["u_nom"] ?>
        </div>
  ...
  <?php if ($i == 4): ?>
    <div class="banni">
      ...
      <p>Vous ne pouvez plus modifier votre profil, ni interagir avec le reste de la plateforme.</p>
      <p>Contactez un membre du bureau pour plus d'informations.</p>
    </div>
  <?php endif; ?>

  <?php if ($i == 5): ?>
    <div class="banni">
      <h2><?php echo $infos['u_prenom']." ".$infos["u_nom"] ?> a été banni !</h2>
      <p>Vous n'avez plus accès à son profil.</p>
      <p>Contactez un membre du bureau pour plus d'informations.</p>
    </div>
  <?php endif; ?>
